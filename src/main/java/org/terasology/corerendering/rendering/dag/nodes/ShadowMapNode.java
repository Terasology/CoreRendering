...
/**
 * This node class generates a shadow map used by the lighting step to determine what's in sight of the main light (sun,
 * moon) and what isn't, allowing the display of shadows cast from said light. TODO: generalize to handle more than one
 * light.
 * <p>
 * Instances of this class: - are enabled and disabled depending on the shadow setting in the rendering config. - in VR
 * mode regenerate the shadow map only once per frame rather than once per-eye.
 * <p>
...
public class ShadowMapNode extends ConditionDependentNode implements PropertyChangeListener {
    ...
    public Camera shadowMapCamera = new OrthographicCamera(-SHADOW_FRUSTUM_BOUNDS, SHADOW_FRUSTUM_BOUNDS,
     ...
     * <p>
     * - DYNAMIC_SHADOWS - SHADOW_MAP_RESOLUTION
     * <p>
     * It assumes the event gets fired only if one of the property has actually changed.
     *
     * @param event a PropertyChangeEvent instance, carrying information regarding what property changed, its
    ...
    /**
     * Re-positions the shadow map camera to loosely match the position of the main light (sun, moon), then writes depth
     * information from that camera into a depth buffer, to be used later to create shadows.
     * <p>
     * The loose match is to avoid flickering: the shadowmap only moves in steps while the main light actually moves
     * continuously.
     * <p>
     * This method is executed within a NodeTask in the Render Tasklist, but its calculations are executed only once per
     * frame. I.e. in VR mode they are executed only when the left eye is processed. This is done in the assumption that
     * we do not need to generate and use a shadow map for each eye as it wouldn't be noticeable.
     */
    @Override
    public void process() {
        ...
        if (worldRenderer.isFirstRenderingStageForCurrentFrame()) {
            ...
            // FIXME: storing chunksOpaqueShadow or a mechanism for requesting a chunk queue for nodes which calls
            //  renderChunks method?
    ...
    private Vector3f getQuantizedMainLightDirection(float stepSize) {
        float mainLightAngle =
                (float) Math.floor((double) backdropProvider.getSunPositionAngle() * stepSize) / stepSize + 0.0001f;
        Vector3f mainLightDirection = new Vector3f(0.0f, (float) Math.cos(mainLightAngle),
                (float) Math.sin(mainLightAngle));
