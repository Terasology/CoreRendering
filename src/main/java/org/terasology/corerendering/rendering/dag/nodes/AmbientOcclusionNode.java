...
/**
 * Instances of this node work in tandem with instances of the BlurredAmbientOcclusionNode class. Together they
 * constitute an ambient occlusion pass.
 * <p>
 * This particular node generates a first, sharper ambient occlusion output. Subsequently that's used by the
 * BlurredAmbientOcclusionNode to make it softer.
 * <p>
 * At this stage only the output of BlurredAmbientOcclusionNode is used to enhance the image eventually shown on screen
 * to the user. It is currently not possible to use the sharper output produced by this node alone, i.e. to have lower
 * quality but faster ambient occlusions.
 * <p>
...
public class AmbientOcclusionNode extends ConditionDependentNode {
    ...
    private final float ssaoStrength = 1.75f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 0.1f, max = 25.0f)
    private final float ssaoRad = 1.5f;
    ...
    public void setDependencies(Context context) {
        ...
        ssaoFbo = requiresFbo(new FboConfig(SSAO_FBO_URI, FULL_SCALE, FBO.Type.DEFAULT),
        ...
        addDesiredStateChange(new SetInputTextureFromFbo(texId++, lastUpdatedGBuffer, DepthStencilTexture,
                displayResolutionDependentFBOs, SSAO_MATERIAL_URN, "texDepth"));
        addDesiredStateChange(new SetInputTextureFromFbo(texId++, lastUpdatedGBuffer, NormalsTexture,
                displayResolutionDependentFBOs, SSAO_MATERIAL_URN, "texNormals"));
        addDesiredStateChange(new SetInputTexture2D(texId, generateNoiseTexture().getId(), SSAO_MATERIAL_URN,
                "texNoise"));
     ...
     * into their own FBOs. The stored images are eventually combined with others.
     * <p>
    ...
    private Texture generateNoiseTexture() {
        ...
        if (!texture.isPresent()) {
            ...
            for (int i = 0; i < SSAO_NOISE_SIZE * SSAO_NOISE_SIZE; ++i) {
                Vector3f noiseVector = new Vector3f(randomGenerator.nextFloat(-1.0f, 1.0f),
                        randomGenerator.nextFloat(-1.0f, 1.0f), 0.0f);
            ...
            return Assets.generateAsset(new ResourceUrn("engine:ssaoNoise"), new TextureData(SSAO_NOISE_SIZE,
                    SSAO_NOISE_SIZE,
