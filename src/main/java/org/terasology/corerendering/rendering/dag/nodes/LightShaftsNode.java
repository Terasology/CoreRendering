...
/**
 * An instance of this class takes advantage of the color and depth buffers attached to the read-only gbuffer and
 * produces light shafts from the main light (sun/moon). It is therefore a relatively inexpensive 2D effect rendered on
 * a full screen quad - no 3D geometry involved.
 * <p>
 * Trivia: the more correct term would be Crepuscular Rays [1], an atmospheric effect. One day we might be able to
 * provide indoor light shafts through other means and it might be appropriate to rename this node accordingly.
 * <p>
...
public class LightShaftsNode extends ConditionDependentNode {
    ...
    private final float exposureDay = 0.0075f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 0.0f, max = 0.01f)
    private final float exposureNight = 0.00375f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 0.0f, max = 10.0f)
    private final float weight = 8.0f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 0.0f, max = 0.99f)
    private final float decay = 0.95f;
    @SuppressWarnings("FieldCanBeLocal")
    private final Vector4f sunPositionWorldSpace4 = new Vector4f();
    @SuppressWarnings("FieldCanBeLocal")
    private final Vector4f sunPositionScreenSpace = new Vector4f();
    ...
    public void setDependencies(Context context) {
        ...
        FBO lightShaftsFbo = requiresFbo(new FboConfig(LIGHT_SHAFTS_FBO_URI, HALF_SCALE, FBO.Type.DEFAULT),
    ...
    /**
     * Renders light shafts, taking advantage of the information provided by the color buffer and especially the depth
     * buffer attached to the FBO currently set as read-only.
     */
    @Override
    public void process() {
        ...
        sunPositionWorldSpace4.set(-sunDirection.x * 10000.0f, -sunDirection.y * 10000.0f, -sunDirection.z * 10000.0f
                , 1.0f);
        ...
        lightShaftsMaterial.setFloat2("lightScreenPos", (sunPositionScreenSpace.x + 1.0f) / 2.0f,
