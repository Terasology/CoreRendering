 ...
 * "engine:sceneReflected" buffer. The content of the buffer is later used to render the reflections on the water
 * surface.
 * <p>
 * This class could potentially be used also for other reflecting surfaces, i.e. metal, but it only works for horizontal
 * surfaces.
 * <p>
 * Instances of this class are not dependent on the Video Settings or any other conditions. They can be disabled by
 * using method Node.setEnabled(boolean) or by removing the instance from the Render Graph.
 */
public class BackdropReflectionNode extends AbstractNode {
    ...
    private final float sunExponent = 512.0f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 1.0f, max = 8192.0f)
    private final float moonExponent = 256.0f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 0.0f, max = 10.0f)
    private final float skyDaylightBrightness = 0.6f;
    @SuppressWarnings("FieldCanBeLocal")
    @Range(min = 0.0f, max = 10.0f)
    private final float skyNightBrightness = 1.0f;
    ...
    /**
     * Renders the sky, reflected, into the buffers attached to the "engine:sceneReflected" FBO. It is used later, to
     * render horizontal reflective surfaces, i.e. water.
     * <p>
     * Notice that this method clears the FBO, both its color and depth attachments. Earlier nodes using the same
     * buffers beware.
     */
    @Override
    public void process() {
        ...
        skyMaterial.setFloat4("skySettings", sunExponent, moonExponent, skyDaylightBrightness, skyNightBrightness,
                true);
