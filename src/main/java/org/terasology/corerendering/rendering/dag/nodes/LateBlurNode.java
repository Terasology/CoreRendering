...
/**
 * This class is a thin facade in front of the BlurNode class it inherits from. The term "late" is due to the fact that
 * this type of nodes is used near the end of the rendering process leading to an image shown on the user display.
 * <p>
 * Given an input FBO a blurred version of it will be stored in the given output FBO. Eventually the blurred version can
 * be used for blur-based effects such as Depth of Field.
 * <p>
 * For more information on Blur: https://en.wikipedia.org/wiki/Box_blur For more information on DoF:
...
public class LateBlurNode extends BlurNode implements PropertyChangeListener {
    ...
    public LateBlurNode(String nodeUri, Name providingModule, Context context, FBO outputFbo) {
        ...
        requiresCondition(() -> renderingConfig.getBlurIntensity() != 0); // getBlurIntensity > 0 implies blur is
        // enabled.
